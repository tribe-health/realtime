defmodule Ewalrus.SubscribersNotification do
  ...
  def broadcast_change(room_id, %{subscription_ids: subs_id} = change, scope, event_filter) do
    payload =
    ...
    encoded_msg =
      %Broadcast{
        topic: "room:chat_messages:" <> room_id,
        event: "realtime",
    ...
    Enum.each(changes, fn change ->
      case change do
        ...
        when is_binary(schema) and is_binary(table) and is_binary(type) ->
          ...
          #   broadcast_change("#{@topic}:*", change, id, %{schema: "*"})
          # end

          # Shout to specific table - e.g. "realtime:public:users"
          # if has_table(event_config, schema, table) do
          #   broadcast_change(table_topic, change, id, %{schema: "public", table: "users"})
          # end
          ...
          if type in ["INSERT", "UPDATE", "DELETE"] do
            ...
            is_map(record) &&
              Enum.each(record, fn {k, v} ->
                with "room_id" <- k,
                     true <- is_notification_key_valid(v),
                     {:ok, stringified_v} <- stringify_value(v),
                     true <- is_notification_key_length_valid(stringified_v) do
                  broadcast_change(stringified_v, change, id, %{
                    schema: "public",
                    table: "users",
                    filter: "#{k}=eq.#{stringified_v}"
                  })
                end
              end)
          end
