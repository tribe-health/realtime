defmodule RealtimeWeb.ExecutionControllerTest do
  ...
  describe "create" do
    ...
    test "returns an error if the log_type is invalid", %{conn: conn} do
      workflow_id = insert_workflow(conn)

      attrs = execution_fixture(%{log_type: "disk"})
      conn = post(conn, Routes.workflow_execution_path(conn, :create, workflow_id), attrs)
      assert %{"errors" => %{"log_type" => ["is invalid"]}} = json_response(conn, 400)
    end
  end
