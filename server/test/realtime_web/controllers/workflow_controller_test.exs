defmodule RealtimeWeb.WorkflowControllerTest do
  ...
  describe "index" do
    test "list all workflows", %{conn: conn} do
      ...
      conn = get(conn, Routes.workflow_path(conn, :index))
      assert [%{"name" => _}] = json_response(conn, 200)["workflows"]
    end
  end
  ...
  describe "create" do
    ...
    test "returns an error if a required field is missing", %{conn: conn} do
      ...
      missing_default_log_type = %{@create_attrs | default_log_type: nil}
      conn = post(conn, Routes.workflow_path(conn, :create), missing_default_log_type)
      assert %{"errors" => %{"default_log_type" => ["can't be blank"]}} = json_response(conn, 400)
    ...
    test "returns an error if the default_log_type is invalid", %{conn: conn} do
      # First check create returns new workflow with correct default_log_type values
      default_log_type_none = %{@create_attrs | default_log_type: "none", name: "Log Type 1"}
      conn = post(conn, Routes.workflow_path(conn, :create), default_log_type_none)
      assert %{"workflow" => _} = json_response(conn, 201)

      default_log_type_postgres = %{@create_attrs | default_log_type: "postgres", name: "Log Type 2"}
      conn = post(conn, Routes.workflow_path(conn, :create), default_log_type_postgres)
      assert %{"workflow" => _} = json_response(conn, 201)

      default_log_type_invalid = %{@create_attrs | default_log_type: "disk"}
      conn = post(conn, Routes.workflow_path(conn, :create), default_log_type_invalid)
      assert %{"errors" => %{"default_log_type" => ["is invalid"]}} = json_response(conn, 400)
    end
